...
def hand():
    ...
    while True:
        ...
        cv2.imshow("Hand Sign Detector", img)
...
def multi():
    ...
    while(True):
        ...
        class_names = [c.strip() for c in open("./models/classes.TXT").readlines()]   #Return a copy of the string with leading a
...
def loginuser():
    msg = ''
    print('aaaaaa')
    if request.method == 'POST' and 'regNumber' in request.form and 'password' in request.form:
        regNumber = request.form['regNumber']
        password = request.form['password']
        print('bbbbbbb')
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'SELECT * FROM users WHERE regNumber = %s AND password = %s', (regNumber, password, ))
        ...
        if account:
            ...
            return render_template('lecture.html', msg = msg)
        else:
...
def logout():
    ...
    session.pop('id', None)
    session.pop('regNumber', None)
    return redirect(url_for('login'))
  
@app.route('/register', methods =['GET', 'POST'])
def register():
    msg = ''
    if request.method == 'POST' and 'regNumber' in request.form and 'firstName' in request.form and 'lastName' in request.form and 'email' in request.form and 'password' in request.form:
        regNumber = request.form['regNumber']
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT * FROM users WHERE regNumber = % s', (regNumber, ))
        account = cursor.fetchone()
        if account:
            msg = 'Account already exists !'
        elif not re.match(r'[^@]+@[^@]+\.[^@]+', email):
            msg = 'Invalid email address !'
        elif not re.match(r'[A-Za-z0-9]+', regNumber):
            msg = 'Username must contain only characters and numbers !'
        elif not regNumber or not password or not email:
            msg = 'Please fill out the form !'
        else:
            cursor.execute(
                'INSERT INTO users VALUES (NULL, % s, % s, %s, % s, % s)', (regNumber, firstName, lastName, email, password, ))
